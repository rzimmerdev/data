...
def get_data(path):
    ...
    for tile in range(1, len(tiles)):
        ...
        for image in images:
            paths.append({"image": f"{path}/{tile}/images/{image}",
                          "label": f"{path}/{tile}/masks/{image}".replace("jpg", "png")})
...
def train_fn(loader: DataLoader, model: torch.nn.Module, optimizer: optim.Optimizer, loss_fn, scaler, epochs, device):
    ...
    Args:
        loader: DataLoader object to get pairs of images - labels for training the predictor
        ...
        scaler: Torch scaler object to accelerate the backward propagation algorithm for the model,
        and it's loss function
        epochs: How many times to generate a new batch of training images and train the model for
        device: On which processing device to run the training batch, can be either 'gpu' or 'cpu'
    """
...
def main(device):
    ...
    try:
        train_fn(dataloader, model, optimizer, loss_fn, grad_scaler, epochs, device)
